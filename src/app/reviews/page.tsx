...
export const metadata: Metadata = {
  ...
  description:
    'Read what families say about Genesis Life Care — rated 4.8★ across our 5 nursing homes on Google. Real reviews from families whose loved ones we care for.',
  alternates: {
    canonical: 'https://genesiscare.com.my/reviews',
    languages: { 'zh-Hans': 'https://genesiscare.com.my/zh/reviews' },
  },
  openGraph: {
    title: 'Reviews & Testimonials | Genesis Life Care',
    description: 'Rated 4.8★ across our 5 nursing homes. Real reviews from families whose loved ones we care for.',
    url: 'https://genesiscare.com.my/reviews',
    siteName: 'Genesis Life Care',
    locale: 'en_MY',
    type: 'website',
    images: [{ url: 'https://genesiscare.com.my/images/services/common-area-scenic.jpg', width: 1200, height: 630, alt: 'Genesis Life Care reviews' }],
  },
};

export const revalidate = 60;

/* ─── Aggregate the real reviews already published on our centre pages ─── */

const all = Object.values(centres);
const reviewList = all.flatMap((c) =>
  (c.testimonials || [])
    .filter((t) => t.text && t.text.trim().length > 0)
    .map((t) => ({ ...t, centre: c.name })),
);
    ...
    <main className="bg-white">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6 text-center">
            {all.map((c, i) => (
              <div key={i}>
                ...
                <p className="text-xs font-bold text-gray-900 uppercase tracking-wider mt-1">{c.name}</p>
                <p className="text-xs text-gray-500 mt-0.5">{c.reviews} reviews</p>
              </div>
            ))}
          </div>
        </div>
              ...
              <div key={i} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Google Review · {r.centre}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="hero-dark bg-gradient-to-br from-primary to-secondary">
        <div className="max-w-4xl mx-auto px-4 py-16 text-center">
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white mb-4" style={{ color: 'white' }}>See why families trust us</h2>
          <p className="text-lg text-white/90 mb-8 max-w-xl mx-auto" style={{ color: 'rgba(255,255,255,0.9)' }}>Book a free visit to any Genesis centre and see the care behind these reviews for yourself.</p>
